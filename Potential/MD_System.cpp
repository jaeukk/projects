...
////#pragma omp single
			...
			//calculate a(t+dt)
			//some potentials need to do pre-computation before computing force
			//these pre-computations may not be thread safe
			//therefore, use only one thread to compute force for particle 0 in advance
////#pragma omp single
			{
				potential.SetConfiguration(this->Position);
				this->_GetAcc(potential);
