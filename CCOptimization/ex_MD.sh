...
K2=0.1 	                   # upper bound on the exclusion region (in unit number density)
...
Nc=20                          # Number of configurations
initial=random                 # Choice of initial condition (random/input)

# MD simulation parameters (Inset them between "MD" and "run" commands)
...
dt="timestep 0.05"			# set dimensionless time step. 0.01 is the default value. When `autotimestep` is used, this value becomes the initial time step.
int_samp="samplesteps 1000" # set sample interval along the MD trajectory. 1e5 is the default value. 
num_equi="numequilibration 10" # set the number of steps in MD simulations spent for equilibration. Those configurations are discarded. If you want to turn on "AutoTlsimeStep", this quantity must be positive!!
auto_dt="autotimestep"		# make the code automatically determine the MD time step. By defualt, it uses a fixed time step.  
restore="restore"			# store the last progress in the MD simulation (whether the prescribed samples are all gathered or not), and load in the future simulations. 
...
echo "${exe} ${timelimit} ${seed} ${beg_idx} ${Verbosity}  <<< \"${d} ${K1a} ${K2a} $S0 $vareps0 ${sigma} ${phi_fake} \
...
${threads} ${N} ${Nc} random ${fname}_MD \
MD $T $dt $int_samp $num_equi $restore run" > log_MD1
...
${exe} ${timelimit} ${seed} ${beg_idx} ${Verbosity}  <<< "${d} ${K1a} ${K2a} $S0 $vareps0 ${sigma} ${phi_fake} \
${threads} ${N} ${Nc} random ${fname}_MD \
MD $T $dt $int_samp $num_equi $restore run" > log_MD2
